...
import { getCollection, getCollectionNftCount, getNftByIdFromCollectionName } from '../../services';
...
const CollectionsId: NextPage = () => {
    ...
    const fetchNftId = (collectionName: string, nonce: number, callback: () => void) => {
        if (nonce && nonce <= nftCount) {
            getNftByIdFromCollectionName(collectionName, nonce).then((response) => {
                if (response.data && Array.isArray(response.data) && Array.from(response.data).length === 1) {
                        ...
                        collectionError ? <p>{collectionError}</p>
                            ...
                            <div>
                                <form onSubmit={handleSubmitSearch} className="mb-4">
                                    <label className="block mb-1">Search <span className='text-primary-500'>{collection?.name}</span> NFT by ID</label>
                                    <input
